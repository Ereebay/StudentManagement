...
public class User {
    ...
    public int login(int k)
    {
        ...
        else
        {
            for (int i = 0; i < list2.size(); i++) {
                if (Objects.equals(user, list2.get(i).getUser())) {
                    if (Objects.equals(password, list2.get(i).getPassword())) {
                        index = i;
                        System.out.println("登陆成功");
                        j = 1;
                        break;
    ...
    public int register(int i)
    {
        ...
        else
        {
            ...
            tea.setPassword(pwd);
            list2.add(tea);
            System.out.println("注册成功，请牢记你的用户名：" + list2.get(list2.size() - 1).getUser() + "密码：" + list2.get(list2.size() - 1).getPassword());
            return 1;
        }
    }
}
